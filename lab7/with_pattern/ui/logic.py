...
from src.pattern import TrayNotifier, EmailNotifier, TelegramNotifier
...
class Menu(QDialog):
    def __init__(
        self, db_manager, ui_dialog, user_id: str, username: str, icon_path: str
    ):
        super().__init__()
        self.ui = ui_dialog
        ...
        self.tray_notifier = TrayNotifier(self)
    ...
    def save_task(self):
        ...
        if self.current_task_id is None:
            ...
            if task_id:
                ...
                self.ui.task_list.addItem(item)
                self.tasks[id(item)] = Task(
                    id=task_id, title=title, description=description, datetime=datetime
                )
            ...
            result = self.db.update_task(
                task.id, title=title, description=description, datetime=datetime
            )
    ...
    def schedule_task_notification(self, task_id):
        ...
        notifier = self.tray_notifier

        user_info = self.db.get_user_info(self.user_id)

        if user_info.get("email_notify") and user_info.get("email"):
            notifier = EmailNotifier(notifier, user_info["email"])

        if user_info.get("telegram_notify") and user_info.get("telegram_id"):
            notifier = TelegramNotifier(notifier, user_info["telegram_id"])
        ...
        if task_dt > current_time:
            ...
            exact_timer.timeout.connect(lambda: notifier.notify(task))
...
class LoginDialog(QDialog):
    ...
    def _validate_credentials(self):
        credentials = {"user_id": None, "username": None, "password": None}
